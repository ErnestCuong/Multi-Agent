...
def get_trend_analysis(slope, initial_value, r_squared):
    # Start as constant trend then check for slope
    trend = Trend.CONSTANT.value

    if slope / initial_value > THRESHOLD_SLOPE_TO_MIN_RATIO:
        # Slightly positive slope
        trend = Trend.INCREASING_SLIGHTLY.value
    if slope / initial_value > 2 * THRESHOLD_SLOPE_TO_MIN_RATIO:
...
def get_variability_analysis(r_squared):
...
def get_goal_analysis(list_of_months, list_of_values, target_pair):
    ...
    if target_comparison == "Lower":
        ...
        return f"{list_of_bad_months} higher than the target {target_value}"
    elif target_comparison == "Higher":
        mask = list_of_values < target_value
        list_of_bad_months = [s for s, b in zip(list_of_months, mask) if b]
        return f"{list_of_bad_months} lower than the target {target_value}"
        ...
        return f"{list_of_higher_months} higher than the target {target_value}, {list_of_lower_months} lower than the target {target_value}"
    return ""
